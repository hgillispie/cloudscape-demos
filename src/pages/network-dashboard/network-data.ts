...
export const generateDeviceData = (): NetworkDevice[] => {
  ...
  return Array.from({ length: 15 }, (_, i) => ({
    ...
    macAddress: `00:1B:63:${Math.floor(Math.random() * 256).toString(16).padStart(2, '0').toUpperCase()}:${Math.floor(Math.random() * 256).toString(16).padStart(2, '0').toUpperCase()}:${Math.floor(Math.random() * 256).toString(16).padStart(2, '0').toUpperCase()}`,
