...
export function CurrentWeather({ weatherData, locationName }: CurrentWeatherProps) {
  ...
  return (
    <Container header={<Box variant="h2">Current Weather - {locationName}</Box>}>
      <ColumnLayout columns={4} variant="text-grid">
        <div>
          ...
          <StatusIndicator type={WeatherApiService.getWeatherIcon(current.weatherCode) as any}>
        ...
        <div>
          ...
          <Box fontSize="body-s">{formatTime(current.time)}</Box>
        </div>
      </ColumnLayout>
    </Container>
  );
